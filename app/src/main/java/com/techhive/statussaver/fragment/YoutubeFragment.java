...
public class YoutubeFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater paramLayoutInflater, ViewGroup paramViewGroup, Bundle paramBundle) {
        ...
        ((MyGalleryActivity) requireActivity()).txtToolbarName.setText(getResources().getString(R.string.youtube));
        return view;
    }
    ...
    void displayfiles(File file, final RecyclerView mRecyclerView) {
        File[] countFile = dirListByAscendingDate(file);
        if (countFile != null) {
            ...
            int i = 0;
            while (i < countFile.length) {
                listOfImages.add(new DataModel(countFile[i].getAbsolutePath(), countFile[i].getName()));
                i++;
            }
        }
